...
for real in range(args.realmin, args.realmax):
    if not (args.mockver is None):
        ...
        elif args.mockver == 'ab_thirdgen':
            mockpath = args.mockpath
            file_name = 'cutsky_{TYPE}_{Z}_{PH}.fits'

            mockdir = os.path.join(args.base_output, 'ThirdGenMocks', Abacus_dir)
            #if args.split_snapshot == 'y':
            create_dir(mockdir)
            if not os.path.isfile(os.path.join(mockdir, 'prepare_mock_arguments.txt')):
                with open(os.path.join(mockdir, 'prepare_mock_arguments.txt'), 'w') as f:
    ...
    datat = []
    for type_ in types:
        if args.mockver == 'ab_firstgen' or args.mockver == 'ab_secondgen' or args.mockver == 'ab_thirdgen':
            print('enter here correct')
            if args.split_snapshot == 'y':
                datas = []

                for bins in zs[type_]:
                    print(bins)
                    thepath = os.path.join(mockpath, bins, file_name.format(TYPE = type_, Z = bins, PH = "%03d" % real))
                    ##TEMPthepath = os.path.join(mockpath, type_, bins, file_name.format(TYPE = type_, Z = bins, PH = "%03d" % real))
                    print('thepath')
                    print(thepath)
                    dat = fitsio.read(thepath, columns=['RA','DEC','Z','Z_COSMO','STATUS', 'MASS', 'ID'])#f[1].data
        ...
        if args.mockver == 'ab_secondgen' or args.mockver == 'ab_thirdgen':
    ...
    del datat
    if args.mockver != 'ab_secondgen' and args.mockver != 'ab_thirdgen':
    ...
    if args.apply_mask == 'y':
        ...
        mask = bitmask.get_nobsandmask(targets, nproc=64)
        maskv = mask.get_nobsandmask(nproc=64)
    ...
    targets.rename_column('ID', 'HALO_ID') 
    targets.rename_column('MASS', 'HALO_MASS') 
