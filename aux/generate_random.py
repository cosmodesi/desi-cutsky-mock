...
def create_random(in_path, random_out_path, tracer, snap, random):
    boxL = 2000

    file_ = in_path + f"/{tracer}_{snap}_ph000.gcat.suball._shell_100.h5py"
    f = h5py.File(file_, 'r')
    length = f.attrs["NGAL"]	
    f.close()
    print(length)
    ...
    for i in range(4):
        for j in range(4):
            for k in range(4):
                ...
                # print(filename)
                data = np.zeros(len(X[range_]), dtype=[('x','f4'), ('y','f4'), ('z','f4')])
