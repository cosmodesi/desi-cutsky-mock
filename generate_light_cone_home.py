...
#                    zi  = self.results.redshift_at_comoving_radial_distance(r / self.h) # interpolated distance from position
                    ...
                    if idx.size!=0:
                        ...
                        if self.mock_random_ic == "mock":
                            ...
                            tot_aux2 = np.append(tot_aux2, idcen[idx])
                            tot_aux3 = np.append(tot_aux3, rvir[idx])
                            tot_aux4 = np.append(tot_aux4, vpeakhost[idx])
                            tot_aux5 = np.append(tot_aux5, logmcen[idx])
    ...
    def generate_shells(self, path_instance, snapshot=None, redshift=None, cutsky=True, nproc=5, n_subboxes=27, cat_seed=None, phase=None):
        ...
        for shellnum in shellnums:
            ...
            with h5py.File(out_file_name_tmp, 'w') as out_file:
                ...
                #TEMPout_file.create_dataset('galaxy/Z_COSMO', data=zz0_array,     dtype=np.float32)



                if self.mock_random_ic == "mock":
                    out_file.create_dataset('galaxy/Z_BAD',   data=aux0_array, dtype=np.float32)
                    #TEMPout_file.create_dataset('galaxy/Z_RSD',   data=aux0_array, dtype=np.float32)
                    out_file.create_dataset('galaxy/IDCEN',   data=aux2_array, dtype=np.int64)
                    out_file.create_dataset('galaxy/RVIR',   data=aux3_array, dtype=np.float32)
                    out_file.create_dataset('galaxy/VPEAKHOST',   data=aux4_array, dtype=np.float32)
                    out_file.create_dataset('galaxy/LOGMCEN',   data=aux5_array, dtype=np.float32)
                    out_file.create_dataset('galaxy/SAT',   data=aux6_array, dtype=np.float32)
