...
from apply_survey_geometry_ic import mask
...
def count(files):
    ...
    for i, file_ in enumerate(files):
        print('this file ',file_)
        f = h5py.File(file_, 'r')
        data = f['galaxy']
        ra_tmp      = data['RA'][()]
        status_tmp  = data['STATUS'][()]
        zcosmo_tmp  = data['Z_COSMO'][()]
        ...
        f.close()
    return counter_TOT, counter_NGC, counter_SGC, np.mean(zcosmo_tmp)
...
def stack_shells(survey_geometry_instance, inpath="test", out_file="test", seed=0, max_seed=0, min_seed=1, mock_random_ic=None, ngc_sgc_tot=None):
    filesT = glob.glob(inpath + "/*hdf5")
    filesT.sort()

    print("INFO: The number of shells: ", len(filesT))

    batch_files = np.array_split(filesT, 7)
    for k,files in enumerate(batch_files):

        print(f"Entering {files}")
        counter_TOT, counter_NGC, counter_SGC, mymeanz = count(files)
        print(f"The number of tracers in  {files} with mean z: {mymeanz}: TOT={counter_TOT}; NGC={counter_NGC}; SGC={counter_SGC}")
        ...
        if mock_random_ic == "mock":
            ...
            add_columns = [('ONEplusDELTA', 'f4'), ('ID', 'i8')]
        ...
        if ngc_sgc_tot == "TOT":
            ...
            fits = fitsio.FITS(out_file + "%d_tmp"%k, "rw")
            fits.write(data_fits_TOT, header=hdict)
            fits.close()
            os.rename(out_file + "%d_tmp"%k, out_file.split('.fits')[0]+'_%d.fits'%k)
            ...
            out_file_NGC = out_file.format(phase=seed) + "%d_Y5_NGC.fits"%k
            out_file_SGC = out_file.format(phase=max_seed - seed + min_seed) + "%d_Y5_SGC.fits"%k

            fits = fitsio.FITS(out_file_NGC+"%d_tmp"%k, "rw")
            fits.write(data_fits_NGC, header=hdict)
            fits.close()
            os.rename(out_file_NGC+"%d_tmp"%k, out_file_NGC)

            fits = fitsio.FITS(out_file_SGC+"%d_tmp"%k, "rw")
            fits.write(data_fits_SGC, header=hdict)
            fits.close()
            os.rename(out_file_SGC+"%d_tmp"%k, out_file_SGC)
