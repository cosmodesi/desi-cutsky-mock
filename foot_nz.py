...
def nz_oneperc(zz, nz_pars):
	if nz_pars['galtype'] == "LRG":
		...
		filename1 = "sm_LRG_main_mycosmo_ev2.1.dat"
		failurerate = 0.02
...
def downsample(boxL, nz_pars, ngalbox, z_cosmo):
...
def generate_shell(args):
	file_, boxL, nz_pars, footprint_mask, todo = args
	# print(file_)
	...
	if todo == 0:
		...
		out_arr, ran_arr = downsample(boxL, nz_pars, f.attrs["NGAL"], z_cosmo)
	elif todo == 2:
		foot_bit = apply_footprint(ra, dec, footprint_mask)
		down_bit, ran_arr = downsample(boxL, nz_pars, f.attrs["NGAL"], z_cosmo)
...
class FOOT_NZ():
	...
	def shell(self, path_instance, nproc=5, footprint_mask=0, todo=1):
		...
		args = product(infiles, [self.boxL], [self.nz_par], [footprint_mask], [todo])
